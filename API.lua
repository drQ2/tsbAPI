...
API.Camera = function() return workspace.CurrentCamera end
API.chr = function()return Player and Player.Character end
API.Humanoid = function()return API.chr()and API.chr():FindFirstChildWhichIsA("Humanoid")end
API.RootPart = function()return API.chr()and API.chr():FindFirstChild("HumanoidRootPart")end
API.Bind(k,c)game:GetService("UserInputService").InputBegan:Connect(function(i,g)if not g and i.KeyCode==k then c()end end)end
API.TP(KEY,POS,DUNK)Bind(KEY,function()if API.RootPart()then API.RootPart().CFrame=DUNK and API.RootPart().CFrame+Vector3.new(0,POS,0)or CFrame.new(POS)end end)end
API.loop(c)coroutine.wrap(function()game:GetService("RunService").RenderStepped:Connect(c)end)()end
API.OnSpawn(c)Player.CharacterAdded:Connect(c)end
...
local function setTransparency(isInvisible)
    for _, part in pairs(API.chr():GetDescendants()) do
        if part:IsA("BasePart")and part.Transparency~=1 then
            part.Transparency=isInvisible and .5 or 0
        end
    end
end
